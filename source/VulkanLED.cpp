...
VkBool32 VulkanLayerAndExtension::areLayersSupported(std::vector<const char *> &layerNames) {
    ...
    auto layerCount = (uint32_t) layerPropertyList.size();
    ...
    for (uint32_t i = 0; i < checkCount; i++) {
        ...
        for (uint32_t j = 0; j < layerCount; j++) {
            if (strcmp(layerNames[i], layerPropertyList[j].properties.layerName) == 0) {
                ...
                supportedLayers.push_back(layerNames[i]);
                isSupported = 1;
                break;
            }
        }
    ...
    auto flag = layerNames.size() == supportedLayers.size();
    layerNames = supportedLayers;
    return flag;
}
